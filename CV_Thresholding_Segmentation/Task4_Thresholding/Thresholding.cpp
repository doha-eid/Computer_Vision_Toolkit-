...
cv::Mat localThresholding(const cv::Mat& grayImage, int windowSize, int threshold)
{
...
//    }
    ...
    cv::Mat outputImage(grayImage.size(), CV_8UC1, cv::Scalar(0));

    // Iterate over the input image, applying the local threshold to each pixel
    for (int y = 0; y < grayImage.rows; y++) {
        for (int x = 0; x < grayImage.cols; x++) {
            // Calculate the local threshold for the current pixel using a sliding window approach
            int sum = 0;
            int count = 0;
            for (int dy = -windowSize/2; dy <= windowSize/2; dy++) {
                for (int dx = -windowSize/2; dx <= windowSize/2; dx++) {
                    int px = std::max(0, std::min(grayImage.cols-1, x+dx));
                    int py = std::max(0, std::min(grayImage.rows-1, y+dy));
                    sum += grayImage.at<uchar>(py, px);
                    count++;
                }
            }
            int localThreshold = sum / count;

            // Convert the input image to a binary image using the local threshold and binary threshold
            if (grayImage.at<uchar>(y, x) > localThreshold + threshold) {
...
cv::Mat globalThresholding(const cv::Mat& inputImage, int threshold)
{
    // Convert the input image to grayscale if it is not already
    cv::Mat grayImage;
    if (inputImage.channels() == 3) {
    ...
    // Iterate over the input image, converting each pixel to a binary value based on the global threshold
    for (int y = 0; y < grayImage.rows; y++) {
        for (int x = 0; x < grayImage.cols; x++) {
